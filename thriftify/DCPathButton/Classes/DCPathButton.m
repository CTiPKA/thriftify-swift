...
- (void)pathCenterButtonFold {
    ...
    for (int i = 0; i < self.itemButtons.count; i++) {
        ...
        CGPoint farPoint = [self createEndPointWithRadius:1.3*self.bloomRadius + 5.0f andAngel:currentAngel/180.0f];
...
- (void)pathCenterButtonBloom {
    ...
    if (self.allowCenterButtonRotation) {
        [UIView animateWithDuration:0.1575f
                         animations:^{
                             _pathCenterButton.transform = CGAffineTransformMakeRotation(0.25f * M_PI);
                         }];
    }
    ...
    for (int i = 0; i < self.itemButtons.count; i++) {
        ...
        CGPoint farPoint = [self createEndPointWithRadius:1.2*self.bloomRadius + 10.0f andAngel:currentAngel/180.0f];
        CGPoint nearPoint = [self createEndPointWithRadius:0.5*self.bloomRadius - 5.0f andAngel:currentAngel/180.0f];
